...
#------------------------------------------------------------------------
# TR: Tadj functionality added 11-Apr-2018.
# Development code for p_ic() with optional Tadj functionality. When this is satisfactorily tested,
# change the function name from p_ict() to p_ic() and deprecate the above. Also change p_ict_inner()
# to p_ic_inner(), deprecating the previous one. This function is called inside p_ict() also, so its
# calls will also need name changes.
#'
#' @title generalized intercensal estimation 
#' 
#' @description Performs intercensal estimation for all intercensal periods containing at least one full calendar year, taking account of universe adjustments (\code{Tadj}).  
#' 
#' @param Pop the standard \code{Pop} object, after age intervals and unknown ages have been dealt with.
#' @param Deaths the standard \code{Deaths} IDB object, after computations finalized.
#' @param Births the standard \code{Births} IDB object, after computations finalized.
#' @param Tadj the standard \code{Tadj} object, as produced by \code{readInputDB()}.
#' @param reproduce.matlab logical. Default \code{FALSE}. Should we reproduce all aspects of the matlab code? 
#' 
#' @details All age intervals must be single before applying this function. Likewise, age intervals greater than one must be split prior to calling this function. Some checking is done to ensure upper ages are high enough. For C1 we must have a single-age estimate for population at age 79 at the lowest. C2 needs a single age estimate for at least age \code{79 + N}, where N is the number of years from C1 to C2, inclusive. If either census has open age groups that are too low, we throw an error and stop the function. Open age groups must be dealt with prior to calling this function in any case. Censuses can be on any date in the year, but territorial adjustments (\code{Tadj}) are dated to January 1 of the specified year (TODO: generalize territorial adjustments for dates). The only Matlab oddity that is potentially affected by \code{reproduce.matlab} is the handling of dates. Matlab does not handle leap years. This function makes calls to other LexisDB functions, such as \code{ypart()}, \code{d_addCohortColumn}, \code{p_addCohortColumn()} and \code{resortPops()}. Most work, and all MP formulas are found in the inner function, \code{p_ic_inner()}, which takes two clean censuses as its inputs. 
#' 
#' @return Pop a standard population data.frame, in single ages, including all years in the intercensal period. \code{C1} and \code{C2} are only included in this object if they were January 1 estimates.
#' 
#' @export 
...
p_ict <- function(Pop, Deaths, Births, Tadj, reproduce.matlab = FALSE){
	...
	for (i in 1:Y){ 
		C1i <- Pop[Pop$Year == C1years[i], ]
		C2i <- Pop[Pop$Year == C2years[i], ]
		# if there's a Tadj we specify it
		if (!missing(Tadj)){
			PIC[[i]] <- p_ict_inner(C1 = C1i, 
					C2 = C2i, 
					Deaths = Deaths, 
					Births = Births, 
					Tadj = Tadj,
			...
			# otherwise no need, objects created on the fly anyway
			PIC[[i]] <- p_ict_inner(C1 = C1i, 
					C2 = C2i, 
					Deaths = Deaths, 
					Births = Births, 
...
#' @title generalized intercensal estimation between two censuses only
#' 
#' @description Called inside \code{p_ic()} on individual census pairs specified to be interpolated. We assume that all age intervals are 1, and we do light checking for upper ages. For C1 we must have a single-age estimate for population at age 79 at the lowest. C2 needs a single age estimate for at least age \code{79 + N}, where N is the number of years from C1 to C2, inclusive. If either census has open age groups that get in the way of this, we throw an error and stop the function. Open age groups must be dealt with prior to calling this function in any case. Likewise, age intervals greater than one must be split prior to calling this function.
#' 
#' @param C1 the standard \code{Pop} object, selected only for rows containing the first (left-side) census.
#' @param C2 the standard \code{Pop} object, selected only for rows containing the second (right-side) census.
#' @param Deaths the standard \code{Deaths} IDB object, after computations finalized.
...
p_ict_inner <- function(C1, C2, Deaths, Births, Tadj, reproduce.matlab = FALSE){
	...
	for (Sex in sexes){ # Sex <- "f"
		...
		if (!missing(Tsex)){
			...
			# not sure about below shift yet
			# TVV              <- rbind(TVV[-1, ],1)
		} 
		...
		# we just did
		##############################################################
		# need to intersect names of DL and DU w TVVdo
		dlc <- sort(intersect(colnames(TVVdo),colnames(DL)))
		duc <- sort(intersect(colnames(TVVdo),colnames(DU)))
		
		# create tadj-adjusted death counts
		# these may need to be age-shifted.
		VVt      <- TVVdo * VV		
		DLt      <- TVVdo[,dlc] * DL
		DUt      <- TVVdo[,duc] * DU
		...
		Db      <- (1 - f1)^2 * DUt[1, ]
		# right-side adjustment for partial years
		Dc      <- f2^2 * DLt[nrow(DLt), ]
		Dd      <- ((2 * f2) - f2^2) * DUt[nrow(DUt), ]
		...
		# which are complete?
		obs     <- colSums(!is.na(DLt))
		Ni      <- as.integer(names(table(obs)))[which.max(table(obs))]
		
		# completed cohorts (C_):
		Ccoh    <- as.integer(colnames(DLt)[obs == Ni])
		Ccoh    <- Ccoh[Ccoh < years[1]]
		Ccoh    <- Ccoh[Ccoh %in% C1s$Cohort & Ccoh %in% C2s$Cohort]
		# infant cohort (I_):
		Icoh    <- years[1]
		...
		# CVV can now be used for cumulative deaths
		CVV              <- VVt[, as.character(Ccoh)]
		CVV[1, ]         <- CDa + CDb
		CVV[nrow(CVV), ] <- CDc + CDd
		
		# get simple starting and ending vectors of population
		# TR: be aware of cohort ordering (ascending or descending)
		# name vectors to be certain we match objects
		C1v              <- C1s$Population
		names(C1v)       <- C1s$Cohort
		C1vec            <- C1v[as.character(Ccoh)]
		# TR: new adjustment for tadj, left side pop
		C1vect           <- TVVdo[1,as.character(Ccoh)] * C1vec
		
		C2v              <- C2s$Population
		names(C2v)       <- C2s$Cohort
		C2vec            <- C2v[as.character(Ccoh)]
		
		
		# assuming no error / migration, we get:
		C2hat            <- C1vec - colSums(CVV)
		
		# remove recent year needs to happen AFTER C2hat, but before cumulative sums
		CVV     <- CVV[-nrow(CVV), ]
		
		# cumulative deaths (already discounted for Da Db, etc)
		CVVcum  <- t(apply(CVV, 2, cumsum))[as.character(Ccoh),]
		...
		# TR: now this may need to Tadjify/ Do IB/IC separately
		# or just do I0? (I0 is called C1 in the MP)
		...
		# tadj adjust deaths, match lengths
		nI      <- length(IVV)
		Ido     <- TVVdo[-nI, as.character(Icoh)]
		IVVt    <- Ido * IVV[-nI]
		I0t     <- I0 * Ido[1]
		IVVcumt <- cumsum(IVVt)
		...
		# estimate population over period
		Ipop    <- I0t - IVVcumt + Iprop * IDelta
		# TR: now undo tadj!!
		Iundo   <- 1 / TVVdo[-nI, as.character(Icoh)]
		Ipop    <- Ipop * Iundo
		# increment years (deaths from prev map to next)
		names(Ipop) <- as.integer(names(Ipop)) + 1
		...
		# Births enter into LDB sorted, so no worries.
		indB    <- Births$Year %in% Ncoh & Births$Sex == Sex
		NB      <- Births$Births[indB]
		names(NB) <- Births$Year[indB]
		
		# these go from C2 backward, so we want to rev them
		indc2   <- C2s$Cohort %in% Ncoh
		N2      <- C2s$Population[indc2]
		names(N2) <-  C2s$Cohort[indc2]
		# match ordering
		N2      <- N2[names(NB)]
		...
		NDc     <- Dc[as.character(Ncoh)]
		NDd     <- Dd[as.character(Ncoh)]
		# TR right side doesn't need adjustment because
		# we take product of vx starting on the right. Dc and Dd
		# sit to the right of the first valid Vx anyway.
		
		# fragile verify this 
		NVV     <- VV[-1, as.character(Ncoh)]
		NVV[nrow(NVV), ] <- NDc + NDd
		NVV[is.na(NVV)] <- 0
		# get tadj object to match 
		Ndo     <- TVVdo[-1, as.character(Ncoh)]
		# adjust NVV
		NVVt    <- NVV * Ndo
		# adjust NB too
		# TR: is this right?
		...
		# estimated pop size, assuming only death decrement
		N2hat  <- NBt - colSums(NVVt, na.rm = TRUE)
		...
		# cumualtive deaths
		NVVcumt <- t(apply(NVVt, 2, cumsum))
		
		# estimate population over period
		Npop    <- NBt - NVVcumt + NDelta
		Npop[NVVcumt == 0] <- NA
		
		# now untadj it
		Nundo <- 1 / TVVdo[rownames(NVVt),colnames(NVVt)]
		Npop  <- Npop * Nundo
		...
		# need AP in the end...
		Ps$Agei        <- Ps$Age <- Ps$Year - Ps$Cohort - 1
		acast(Ps,Agei~Year,value.var="Population")
		# now add on the remaining columns...
		Ps$Sex         <- Sex
		Ps$PopName     <- unique(C1s$PopName)
		Ps$Area        <- unique(C1s$Area)
...
#      Ps <- rbind(p_addCohortColumn(C1[C1$Sex == Sex, ]), Ps)
#    }
# TR: 11-Apr-2018 inner function doesn't need to save C1. That can happen outside this function.
