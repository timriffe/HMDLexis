...
#' 
#' @details This function does not yet deal with territorial adjustments. This is a version that Celeste modified to deal with 0 or low counts.
#' 
...
d_s1x1_CDW <- function(Deaths, Births){
  ...
  #--------------------------------------------------
  # CDW April 29, 2017
  # Create IMR(t) for combined sex using same code as Tim's
  # In order to minimally modify the code, I break the sex-loop
  # into two different loops - one to compute the sex-specific IMR(t)
  # numerators and denominators, which are then combined
  # into the combined-sex IMR(t).  Then, the sex-loop is started
  # anew to compute the rest of the necessary variables for splitting
  # single-year-of-age rectangles
  ...
  IMRnum <- list()
  #--------------------------------------------------
  # sex-loop to compute components of IMR(t)
  for (Sex in c("f","m")){
    ...
    # stuff needed: IMR, pib
    # needs tadj modification, also needs to account for if B(t-1) not available in Birth counts.
    Ball            <- Births$Births[Births$Sex == Sex]
    names(Ball)     <- Births$Year[Births$Sex == Sex]
#    
    IMRdenom[[Sex]]        <- 1 / 3 * Ball[as.character(yrs - 1)] + 2 / 3 * Ball[as.character(yrs)]
   
    # this would only kick in if births from year t-1 are missing. Then assume same as present year
    NAind           <- is.na(IMRdenom[[Sex]])
    IMRdenom[[Sex]][NAind] <- Ball[as.character(yrs[NAind])]
    ...
    IMRnum[[Sex]]          <- acast(Dsex[Dsex$Lexis %in% c("TL","TU","RR") & Dsex$Agei == 0, ], 
                             Age ~ Year, 
                             sum, 
                             value.var = "Deaths", 
                             fill = 0)[ , , drop = TRUE] # drop = TRUE turns it into a vector (named still)
  }
  ## Step outside of sex-specific loop to add the sex-specifc IMR numerators
  ## and denominators, respectively, then calculate IMR and IMT(t)
  IMRnum0 <- IMRnum[["m"]] + IMRnum[["f"]]
  IMRdenom0 <- IMRdenom[["m"]] + IMRdenom[["f"]]
  IMR             <- IMRnum0[as.character(yrs)] / IMRdenom0 # ensures we get same years as need for RR
  ...
  #-----------------------------------------------
  # Sex-loop for computing sex-specific PIb and PId
  for (Sex in c("m","f")) {
    Dsex            <- Deaths[Deaths$Sex == Sex, ]                                                  
    RRi             <- with(Dsex, Lexis == "RR" & AgeInterval == "1" & YearInterval == 1)
    RR              <- Dsex[RRi, ]
